...
class Devices {
    ...
    //
    //
    NewBlinds(client, name) {
        this.debug('Devices:NewBlinds(): BEGIN');
        ...
        if (this.registerDevice(client, device)) {
            this.debug('Devices:NewBlinds(): END ok');
            return JSON.parse(JSON.stringify(states));
        } else {
            this.debug('Devices:NewBlinds(): END fail');
            return {};
        }
    }
    //
